...
function AddTemplateProposalForm() {
  ...
  useEffect(() => {
    if (challengeData) {
      setFundingStateArray(Array.from(new Set(challengeData?.Challenge.map((challenge: any) => challenge.fund))));
    }
  }, [challengeData]);
  ...
  useEffect(() => {
    setFilteredChallenges(challengeData?.Challenge?.filter((challenge: any) => challenge.fund === fundingState));
  }, [fundingState]);
  ...
  //     });
  //   } else {
  //     const graphResponse = await createProposals();
  ...
  const handleDraftSubmit = async (event: { preventDefault: () => void }) => {
    ...
    if (
      ...
      const graphResponse = await createDraftProposals();
      Swal.fire({
    ...
    <>
      <form onSubmit={handleDraftSubmit}>
        ...
        >
          ...
          <Typography sx={{ fontSize: '24px', fontWeight: 'bold', width: '100%', lineHeight: '16px' }}>Proposal Template</Typography>
          {/* TITLE START*/}
          ...
          <Grid item xs={12} className={Style.gridItem}>
            <FormControl fullWidth>
              <Typography>Describe your solution to the problem (140 characters) *</Typography>
              <FilledInput
                required
                fullWidth
                inputProps={{ maxLength: 140 }}
          ...
          <Grid item xs={12} className={Style.gridItem}>
            <Box>
              <FormControl fullWidth sx={{ borderRadius: '4px', mb: 5 }}>
                <Typography>Detailed plan - Fill in here any additional details *</Typography>
                <TextField
...
export default AddTemplateProposalForm;
